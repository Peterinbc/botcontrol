...
resource "aws_acm_certificate" "bcgoatacmcert" {
  domain_name = var.domainnaim
  validation_method = "DNS"
}

resource "aws_route53_record" "bcgoatr53acmcnamerecord" {
  for_each = {
    for dvo in aws_acm_certificate.bcgoatacmcert.domain_validation_options : dvo.domain_name => {
